...
import { INDUSTRY_TO_SECTOR, SECTOR_GROUPS, ALL_INDUSTRIES } from '@/lib/industry-sector-mapping';
...
interface IndustryMatrixProps {
  industryGroups: Record<string, Stock[]>;
}

// Color mapping derived from sectors
const SECTOR_COLORS: Record<string, string> = {
  ...
  'N/A': 'from-slate-500/20 to-gray-500/20 border-slate-500/30',
};
...
export default function IndustryMatrix({ industryGroups }: IndustryMatrixProps) {
  ...
  const getIndustrySize = (stocks: Stock[]) => {
    const stockCount = stocks.length;
    // Scale size based on number of stocks for optimal space utilization
    if (stockCount >= 20) return 'xlarge';
    if (stockCount >= 10) return 'large';
    if (stockCount >= 5) return 'medium';
    if (stockCount >= 1) return 'small';
    return 'tiny';
  };

  const getIndustrySizeClass = (size: string) => {
    switch (size) {
  ...
  // Create industry entries with their parent sectors
  const allIndustries = ALL_INDUSTRIES.map(industry => {
    const sector = INDUSTRY_TO_SECTOR[industry] || 'N/A';
    return [
      industry,
  ...
  // Sort industries within each sector by stock count
  const industriesBySector = allIndustries.sort((a, b) => {
    const aCount = a[1].length;
    const bCount = b[1].length;
    const aSector = a[2];
    const bSector = b[2];
    
    // Both have stocks: sort by sector first, then by count
    if (aCount > 0 && bCount > 0) {
      if (aSector !== bSector) return aSector.localeCompare(bSector);
      return bCount - aCount;
    }
    
    // Both empty: sort by sector, then alphabetically
    if (aCount === 0 && bCount === 0) {
      if (aSector !== bSector) return aSector.localeCompare(bSector);
      return a[0].localeCompare(b[0]);
    }
    
    // Move empty industries to end
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-slate-900/50 backdrop-blur-sm border border-slate-800/50 rounded-xl p-6">
          ...
          <div className="text-white text-3xl font-bold">
            {Object.keys(industryGroups).length}
          </div>
        </div>
        <div className="bg-slate-900/50 backdrop-blur-sm border border-slate-800/50 rounded-xl p-6">
          <div className="text-slate-400 text-sm mb-1">Total Stocks</div>
          <div className="text-white text-3xl font-bold">
            {Object.values(industryGroups).reduce((sum, stocks) => sum + stocks.length, 0)}
          </div>
        </div>
      </div>

      {/* Industry Matrix - Grouped by Sectors */}
      <div className="space-y-4">
        {Object.entries(SECTOR_GROUPS).map(([sectorName, sectorIndustries]) => {
          const industriesInSector = industriesBySector.filter(
            ([industry]) => sectorIndustries.includes(industry) && industryGroups[industry]?.length > 0
          );
          
          if (industriesInSector.length === 0) return null;

          const sectorColorClass = SECTOR_COLORS[sectorName] || 'from-slate-500/20 to-gray-500/20 border-slate-500/30';
          const sectorTextColor = SECTOR_TEXT_COLORS[sectorName] || 'text-slate-400';

          return (
            <div key={sectorName} className="space-y-3">
              {/* Sector Header */}
              <div className="flex items-center gap-3">
                <div className={`h-1 flex-1 bg-gradient-to-r ${sectorColorClass.split(' ')[0]} rounded-full`} />
                <h2 className={`text-xl font-bold ${sectorTextColor}`}>
                  {sectorName}
                </h2>
                <div className="text-slate-500 text-sm">
                  {industriesInSector.reduce((sum, [, stocks]) => sum + stocks.length, 0)} stocks
                </div>
                <div className={`h-1 flex-1 bg-gradient-to-r ${sectorColorClass.split(' ')[0]} rounded-full`} />
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4 auto-rows-fr">
                {industriesInSector.map(([industry, stocks]) => {
                  const size = getIndustrySize(stocks);
                  const sizeClass = getIndustrySizeClass(size);
                  const colorClass = sectorColorClass;
                  const textColor = sectorTextColor;
                  ...
                  return (
                    ...
                    >
                      <div className="flex flex-col h-full">
                        {/* Industry Header */}
                        <div className="mb-4">
                          <h3 className={`text-lg font-bold ${textColor} mb-1 group-hover:text-white transition-colors line-clamp-2`}>
                            {industry}
                          </h3>
      ...
      {selectedIndustry && industryGroups[selectedIndustry] && (
        <div className="bg-gradient-to-br from-slate-900/90 to-slate-950/90 backdrop-blur-sm border border-slate-800/50 rounded-xl p-6">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h3 className="text-2xl font-bold text-white">{selectedIndustry}</h3>
              <p className="text-slate-400 text-sm mt-1">
                Sector: {INDUSTRY_TO_SECTOR[selectedIndustry] || 'N/A'}
              </p>
            </div>
            <button
